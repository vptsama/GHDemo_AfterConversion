 ...
 * Combination of com.google.android.gms.maps.UiSettings and com.huawei.hms.maps.UiSettings.<br/>
...
public final class UiSettings extends org.xms.g.utils.XObject {
    ...
    /**
     * org.xms.g.maps.UiSettings.setIndoorLevelPickerEnabled(boolean) Sets whether the indoor level picker is enabled when indoor mode is enabled. If true, the level picker will appear when a building with indoor maps is focused. If false, no level picker will appear - an application will need to provide its own way of selecting levels. The default behaviour is to show the level picker.<br/>
     * Support running environments including both HMS and GMS which are chosen by users.<br/>
     * Below are the references of HMS apis and GMS apis respectively:<br/>
